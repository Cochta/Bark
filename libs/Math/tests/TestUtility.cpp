/**
 * @brief Unit tests for Maths::Utility functions
...
TEST_P(FunctionsFixture, Abs){
    ...
    EXPECT_EQ(Maths::Utility::Abs<int>(-1 * static_cast<int>(values)), static_cast<int>(values));
    EXPECT_EQ(Maths::Utility::Abs<int>(static_cast<int>(values)), static_cast<int>(values));
    EXPECT_EQ(Maths::Utility::Abs<int>(0), 0);

    EXPECT_EQ(Maths::Utility::Abs<float>(-1 * values), values);
    EXPECT_EQ(Maths::Utility::Abs<float>(values), values);
}
...
TEST_P(FunctionsFixture, Clamp)
{
    ...
    EXPECT_EQ(Maths::Utility::Clamp<int>(static_cast<int>(values), 0, 10), static_cast<int>(result));
    ...
    EXPECT_FLOAT_EQ(Maths::Utility::Clamp<float>(values, 0.0001f, 0.f), result);
}
...
TEST_P(FunctionsFixture, Min)
{
    ...
    EXPECT_EQ(Maths::Utility::Min<int>(static_cast<int>(values), 20), static_cast<int>(result));
    EXPECT_EQ(Maths::Utility::Min<int>(20, static_cast<int>(values)), static_cast<int>(result));
    EXPECT_EQ(Maths::Utility::Min<int>(-0, 0), -0);
    result = values < 0.001f ? values : 0.001f;
    EXPECT_FLOAT_EQ(Maths::Utility::Min<float>(values, 0.001f), result);
}
...
TEST_P(FunctionsFixture, Factorial)
{
    ...
    for (int i = 2; i <= Maths::Utility::Abs(values); i++)
    {
        result *= i;
    }
    ...
    EXPECT_EQ(Maths::Utility::Fact<int>(0), 1);
    EXPECT_EQ(Maths::Utility::Fact<int>(static_cast<int>(values)), static_cast<int>(result));
    EXPECT_FLOAT_EQ(Maths::Utility::Fact<int>(values), result);
}
...
TEST_P(FunctionsFixture, PowInt)
{
    ...
    EXPECT_EQ(Maths::Utility::Pow<int>(values, 5), result);
}
...
TEST_P(FunctionsFixture, PowFloat)
{
    ...
    EXPECT_FLOAT_EQ(Maths::Utility::Pow<float>(values, 5), result);
}
...
TEST_P(RadianTestFixture, Sin)
{
    ...
    for (int i = 0; i < 720; i++)
    {
        EXPECT_NEAR(Maths::Utility::Sin(angles[i]), std::sin(static_cast<float>(angles[i])), epsilon);
    }
}
...
TEST_P(RadianTestFixture, Cos)
{
    ...
    for (int i = 0; i < 720; i++)
    {
        EXPECT_NEAR(Maths::Utility::Cos(angles[i]), std::cos(static_cast<float>(angles[i])), epsilon);
    }
}
...
TEST_P(RadianInTanRangeTestFixture, Tan)
{
    ...
    for (int i = 0; i < 100; i++)
    {
        EXPECT_NEAR(Maths::Utility::Tan(angles[i]), std::tan(static_cast<float>(angles[i])), epsilon);
    }
}
...
std::vector<Radian> GetRadianAngles()
{
    ...
    constexpr float rangeStart = Maths::Utility::TanMargin;
    float value = rangeStart + static_cast<float>(errorMargin) * Maths::Utility::CotStep;
...
TEST_P(AngleFixture, CotWithLUT)
{
    ...
    for (auto a : angles)
    {
        EXPECT_NEAR(Maths::Utility::Cot(a), 1 / std::tan(static_cast<float>(a)), 0.001f);
    }
}