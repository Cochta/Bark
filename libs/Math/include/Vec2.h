...
#pragma region Operators
    ...
    constexpr Vec2<T> operator+(const Vec2<T> vec) const noexcept
    {
	...
	constexpr Vec2<T> operator*(const T scalar) const noexcept
    {
...
#pragma endregion
	...
	[[nodiscard]] constexpr T Dot(const Vec2<T> vec) const noexcept
    {
        return X * vec.X + Y * vec.Y;
    }

	constexpr static T Dot(const Vec2<T> vec1, const Vec2<T> vec2) noexcept
    {
	...
	[[nodiscard]] constexpr bool IsPerpendicular(const Vec2<T> vec) const noexcept
	{
	...
	[[nodiscard]] constexpr bool IsParallel(const Vec2<T> vec) const noexcept
	{
	...
	template<typename U = T>
	[[nodiscard]] constexpr Vec2<U> Project(const Vec2<T> vec) const
	{
	...
	template<typename U = T>
	[[nodiscard]] constexpr Vec2<U> Reflect(const Vec2<T> vec) const
	{
		// a - 2 * (a - b^) * b^
		auto vecA = static_cast<Vec2<U>>(*this);
		auto vecB = static_cast<Vec2<U>>(vec);
	...
	[[nodiscard]] static Radian Angle(const Vec2<T> vec1, const Vec2<T> vec2)
	{
		// acos((a * b) / (|a| * |b|))
	...
	[[nodiscard]] constexpr Radian Angle(const Vec2<T> vec) const
	{
		return Angle(*this, vec);
	}

	template <typename U = T>
	[[nodiscard]] constexpr static Vec2<U> Lerp(const Vec2<T> vec1, const Vec2<T> vec2, float t) noexcept
	{
	...
	template <typename U = T>
	[[nodiscard]] constexpr Vec2<U> Lerp(const Vec2<T> vec, float t) const noexcept
	{
		return Lerp<U>(*this, vec, t);
	}

	template <typename U = T>
	[[nodiscard]] constexpr static Vec2<U> Slerp(const Vec2<T> vec1, const Vec2<T> vec2, float t)
	{
		auto vec1u = static_cast<Vec2<U>>(vec1);
		auto vec2u = static_cast<Vec2<U>>(vec2);
		...
		float sinAngleT = MathUtility::Sin(angleBetween * t);
		float sinAngle1T = MathUtility::Sin(angleBetween * (1 - t));
	...
	template <typename U = T>
	[[nodiscard]] constexpr Vec2<U> Slerp(const Vec2<T> vec, float t) const
	{
		return Slerp(*this, vec, t);
	}

	[[nodiscard]] static float Distance(const Vec2<T> vec1, const Vec2<T> vec2) noexcept
	{
		return (vec1 - vec2).Length();
	}

	[[nodiscard]] float Distance(const Vec2<T> vec) const noexcept
	{
